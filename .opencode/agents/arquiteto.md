...
1. **Codificador** (sessão separada, agente `codificador`) — roda `bash .opencode/scripts/start_task.sh <N>`, que cria a worktree isolada em `.opencode/worktrees/<tipo>-<N>-<slug>/` já na branch `<tipo>/<N>-<slug>`. Executa o plano **na ordem TDD** que você definiu (testes primeiro → RED → código → GREEN). Roda `bash .opencode/scripts/finish_task.sh <N>` que exige `mvn test` verde e zero warnings, gera `SUMMARY.md`. Não comita final, não faz push, não toca `doc/**`.
2. **Revisor** (sessão separada, agente `revisor`) — roda `bash .opencode/scripts/load_review.sh <N>`, audita em 3 camadas (aderência ao plano / qualidade técnica / critérios de aceitação). Se APROVADO: `bash .opencode/scripts/approve.sh <N>` que squasha o commit, faz push e abre PR DRAFT com `Closes #<N>`. Se AJUSTES: `bash .opencode/scripts/reject.sh <N> "<motivo>"` que escreve `REVIEW.md` na worktree.
...
- `.opencode/agents/codificador.md` — próximo elo da cadeia (entenda o que ele espera receber).
- `.opencode/agents/revisor.md` — quem audita o resultado (entenda os critérios que ele aplicará).
- `.opencode/scripts/start_task.sh` — o script que consome as labels que você define.
